...
const UploadVideoPage: React.FC = () => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      // Validation 2: File Size (20MB Limit for Browser Demo Analysis)
      // This ensures we don't try to send a payload that will instantly fail at the API level
      const maxSize = 20 * 1024 * 1024;
      if (selectedFile.size > maxSize) {
        showNotification('error', 'File is too large. Maximum size is 20MB for the browser demo analysis path.');
        setFile(null);
        e.target.value = '';
        return;
      }
    ...
    <TeacherSidebar>
      <header className="mb-8">
        ...
        <p className="text-gray-500 mt-2">
          Upload an English lesson video. TeachNova will extract teaching style, lesson concepts, and a tutor persona.
        </p>
      </header>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-1 text-center">
                  ...
                  <p className="text-xs text-gray-500">
                    {file 
                      ...
                      : 'MP4, MOV, WEBM up to 20MB (browser demo limit)'}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="pt-4">
              <Button type="submit" variant="teacher" isLoading={loading}>
                {loading ? 'Uploading to TeachNova...' : '📤 Upload to TeachNova'}
              </Button>
            </div>
          </form>
        </Card>
