...
import { Message, AIPersonality, ChatAttachment } from '../types';
import { ArrowLeft, Send, Bot, MoreVertical, Mic, MicOff, Play, Pause, Volume2, Paperclip, X, Sparkles } from 'lucide-react';
...
const StudentChatPage: React.FC = () => {
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    if ((!inputText.trim() && !attachment) || !conversationId || !personalityId || isSending) return;

    const text = inputText.trim() || 'Please explain this attached image in the teacher style.';
    ...
    // Call API
    const response = await processStudentMessage(conversationId, parseInt(personalityId), text, selectedAttachment || undefined);
  ...
  const handleAttachmentChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowed.includes(file.type)) {
      showNotification('error', 'Only PNG, JPG, and WEBP images are supported in chat right now.');
      e.target.value = '';
      return;
    }

    if (file.size > 3.75 * 1024 * 1024) {
      showNotification('error', 'Image must be 3.75MB or smaller for Bedrock Converse image input.');
      e.target.value = '';
      return;
    }

    setAttachment({ file, mimeType: file.type, name: file.name });
    showNotification('info', `Attached ${file.name}`);
  };
    ...
    <div className="flex flex-col h-[100dvh] bg-[#e5ddd5] relative">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="inline-flex items-center gap-1 rounded-full bg-purple-50 text-purple-700 border border-purple-100 px-2 py-0.5 text-[11px] font-semibold">
                <Sparkles className="w-3 h-3" /> Powered by Amazon Nova
              </span>
            </div>
          </div>
        </div>
          ...
          return (
            <div key={idx} className={`flex w-full ${isStudent ? 'justify-end' : 'justify-start'}`}>
              <div className={`flex flex-col max-w-[85%] sm:max-w-[70%]`}>
                ...
                >
                    {msg.attachmentName && (
                      <div className="text-[11px] text-blue-600 mb-1 font-medium">📎 {msg.attachmentName}</div>
                    )}
      ...
      <footer className="bg-white px-3 py-2 md:px-4 md:py-3 z-10 shadow-lg border-t border-gray-100">
        <form onSubmit={handleSendMessage} className="max-w-4xl mx-auto">
          {attachment && (
            <div className="mb-2 inline-flex items-center gap-2 bg-blue-50 text-blue-700 border border-blue-100 rounded-full px-3 py-1 text-xs font-medium">
              <span>📎 {attachment.name}</span>
              <button type="button" onClick={clearAttachment} className="text-blue-500 hover:text-blue-700">
                <X className="w-3 h-3" />
              </button>
            </div>
          )}
          ...
          <button
            ...
            className="p-3 rounded-full flex-shrink-0 transition-all shadow-md bg-gray-100 text-gray-600 hover:bg-gray-200"
            title="Attach image"
          >
          ...
          <div className="flex-1 bg-gray-100 rounded-2xl px-4 py-2 border border-transparent focus-within:border-green-500 focus-within:bg-white transition-colors">
            <textarea
              ...
              placeholder={isRecording ? "Listening..." : attachment ? "Add a question about the image..." : "Type your question in English..."}
          ...
          <button 
            type="submit"
            disabled={(!inputText.trim() && !attachment && !isRecording) || isSending}
            className={`
              p-3 rounded-full flex-shrink-0 transition-all shadow-md
              ${(!inputText.trim() && !attachment && !isRecording) || isSending 
